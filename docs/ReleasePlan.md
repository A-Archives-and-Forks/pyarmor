...
- Convert unsupported node types as far as possible for BCC mode (cancelld)
- Expert users could customize C sources generated by BCC mode (cancelld)
- BCC mode support cross-platform (may not if too complex) (cancelled)
- Obfuscate string constants in extension module "pyarmor_runtime" (cancelled)
...
- `pyarmor build` supports option `--pack` to pack project to one executable
- RFT mode supports module name refactor
